...
if(e.target.classList.contains('modal') && e.target.classList.contains('show')) closeModal(e.target.id);
if(e.target.classList.contains('lightbox') && e.target.classList.contains('show')) document.getElementById('photoLightbox').classList.remove('show');
});

window.loader = {
show: msg => { const el=document.getElementById('globalLoader'); document.getElementById('loaderText').textContent=msg||'Memproses...'; el.style.display='flex'; setTimeout(()=>el.classList.add('show'), 10); },
hide: delay => setTimeout(()=>{ const el=document.getElementById('globalLoader'); el.classList.remove('show'); setTimeout(()=>el.style.display='none', 300); }, delay||800)
};

window.addEventListener('DOMContentLoaded', ()=>{ document.getElementById('globalLoader').style.display='none'; document.getElementById('globalLoader').classList.remove('show'); document.querySelectorAll('.modal, .lightbox').forEach(el=>{ el.classList.remove('show'); el.classList.add('hidden'); }); });
...
if(res.success) {
user=res.user; document.getElementById('loginPage').classList.add('hidden'); document.getElementById('dashPage').classList.remove('hidden');
...
function togglePass() { const i=document.getElementById('uPass'), ic=document.getElementById('eyeIcon'); if(i.type==='password'){i.type='text'; ic.classList.replace('fa-eye','fa-eye-slash');}else{i.type='password'; ic.classList.replace('fa-eye-slash','fa-eye');} }

function showPage(id, el) {
document.querySelectorAll('.page').forEach(p=>p.classList.add('hidden')); document.querySelectorAll('.menu-item').forEach(m=>m.classList.remove('active'));
...
s1.innerHTML='<option value="">Semua Seksi</option>'; s2.innerHTML='<option value="ALL">Semua Seksi</option>';
sek.data.forEach(s => { 
...
document.getElementById('statBox').innerHTML=`
    <div class="stat-card">
        ...
        <div><h3>${d.length}</h3><p>Total Hari Ini</p></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon"><i class="fas fa-user-check"></i></div>
        <div><h3>${d.filter(g=>g.status==='CHECKED_IN').length}</h3><p>Sedang Bertamu</p></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon" style="background:#fff7ed;color:#d97706;"><i class="fas fa-building"></i></div>
        <div><h3>${d.filter(g=>g.jenis==='instansi').length}</h3><p>Dari Instansi</p></div>
    </div>
`;

document.getElementById('tblRecent').innerHTML=d.slice(0,8).map(g=>`
    <tr>
        <td>${new Date(g.waktuIn).toLocaleTimeString()}</td>
        <td>${g.nama}</td>
        <td>${g.instansi}</td>
        <td>${g.tujuanSeksi}</td>
...
document.getElementById('tblAll').innerHTML=list.map(g=>{
    ...
    return `<tr>
        <td>${foto}</td>
        <td style="font-family:monospace; color:#666;">${g.id}</td>
        <td style="font-weight:500;">${g.nama}</td>
        <td>${g.tujuanSeksi}</td>
        <td style="font-size:0.8rem;">${new Date(g.waktuIn).toLocaleString()}</td>
        <td><span class="badge ${g.status==='CHECKED_IN'?'badge-in':'badge-out'}">${g.status}</span></td>
...
async function confirmCheckout(id, nama) { showActionModal('confirm', 'Checkout',  `Proses checkout untuk ${nama}?`, async()=>{ const r=await fetchAPI('checkout', {id}); if(r.success){showActionModal('info','Berhasil',r.message); loadGuests(); loadStats();} else showActionModal('info','Gagal',r.message);  }); }

function showActionModal(type, title, msg, onConfirm) {
const m=document.getElementById('actionModal'); m.querySelector('.modal-content-box').style.pointerEvents='auto';
document.getElementById('amTitle').textContent=title; document.getElementById('amMsg').textContent=msg;
const ic=document.getElementById('amIcon'); ic.className= `modal-icon ${type==='confirm'?'warn':'info'}` ; ic.innerHTML=type==='confirm'?'<i class="fas fa-exclamation"></i>':'<i class="fas fa-check-circle"></i>';
document.getElementById('amConfirm').classList.toggle('hidden', type!=='confirm'); document.getElementById('amOk').classList.toggle('hidden', type==='confirm');
...
e.preventDefault(); const btn=document.getElementById('btnSaveUser'); btn.disabled=true; btn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Menyimpan...';
try {
const res=await fetchAPI('addUser', {nama:document.getElementById('nuNama').value, username:document.getElementById('nuUser').value, password:document.getElementById('nuPass').value, role:document.getElementById('nuRole').value, seksiId:document.getElementById('nuSeksi').value});
if(res.success) { closeModal('userModal'); showActionModal('info','Berhasil','User berhasil ditambahkan.'); loadUsers(); }
...
function confirmDeleteUser(id, nama) { showActionModal('confirm', 'Hapus User',  `Yakin hapus user "${nama}"?`, async()=>{ const r=await fetchAPI('deleteUser', {userId:id}); if(r.success){showActionModal('info','Berhasil','User dihapus.'); loadUsers();} else showActionModal('info','Gagal',r.message) ; }); }
...
document.getElementById('tblUsers').innerHTML=res.data.map(u=>`
    <tr>
        ...
        <td>${u.seksiId}</td>
        <td>${u.id!=='U001'?`<button class="btn btn-danger btn-sm" onclick="confirmDeleteUser('${u.id}','${u.nama}')"><i class="fas fa-trash"></i></button>`:'<span style="color:#aaa; font-size:0.7rem;">Protected</span>'}</td>
    </tr>
`).join('');
}

async function genReport(type) {
const out=document.getElementById('reportOut'); out.innerHTML='<p style="color:var(--primary);"><i class="fas fa-spinner fa-spin"></i> Memuat...</p>';
const res=await fetchAPI('generateReport', {role:user.role, seksiId:user.seksiId});
const s=res.stats;
out.innerHTML=`
    ...
    <table>
        <thead><tr><th>Nama</th><th>Tujuan</th><th>Waktu</th><th>Status</th></tr></thead>
        <tbody>
        ${res.data.map(g=>`<tr><td>${g.nama}</td><td>${g.tujuanSeksi}</td><td>${new Date(g.waktuIn).toLocaleString()}</td><td>${g.status}</td></tr>`).join('')}
        </tbody>
    </table>
`;
}
...
document.getElementById('tblSatkers').innerHTML=res.data.map(s=>`
    <tr>
        ...
        <td>${s.nama}</td>
        <td>${s.tipe}</td>
        <td><button class="btn btn-sm" onclick="alert('${s.qrData}')">Lihat</button></td>
    </tr>
...
closeModal('satkerModal'); showActionModal('info','Berhasil','Satker & QR berhasil dibuat.'); loadSatkers();
}
}
